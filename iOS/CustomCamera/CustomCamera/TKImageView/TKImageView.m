...
- (void)handleCropAreaPinch: (UIPinchGestureRecognizer *)pinchGesture {
    ...
    switch (pinchGesture.state) {
        case UIGestureRecognizerStateBegan: {
            ...
            if ([_delegate respondsToSelector:@selector(imageViewDidBeginChangeCropArea:)]) {
                [_delegate imageViewDidBeginChangeCropArea:self];
            }
            break;
        }
        ...
        case UIGestureRecognizerStateEnded:
            if ([_delegate respondsToSelector:@selector(imageViewDidEndChangeCropArea:)]) {
                [_delegate imageViewDidEndChangeCropArea:self];
            }
        default:
            break;
    }
...
- (void)resetCropAreaByAspectRatio {
    ...
    _cropAreaView.frame = CGRectMake((WIDTH(_imageView) - width) / 2.0, (WIDTH(_imageView) - width) / 2.0, width, height);
    [self resetCornersOnCropAreaFrameChanged];
    [self resetCropTransparentArea];
